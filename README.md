...
5、如果Bean实现了ApplicationContextAware接口，则调用该接口的setApplicationContext(ApplicationContext applicationContext)方法，设置applicationContext（可以通过这个特性给某个类的静态方法注入spring bean，要搞清楚IOC容器启动和获得bean是两个步骤。）
...
并在创建任何非BeanFactoryPostProcessor类型bean之前创建并应用它们（例如：配置文件中注册了一个自定义BeanFactoryPostProcessor类型的bean，
一个User类型的bean，应用程序上下文会在创建User实例之前先创建BeanFactoryPostProcessor类型的bean（在容器启动时就创建好了）。
在创建User实例之前对User应用BeanFactoryPostProcessor的接口方法）。
...
## Spring ApplicationContext容器refresh过程（扩展原始beanFactory容器功能）简析
